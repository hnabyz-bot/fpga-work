#!/bin/bash
#==============================================================================
# init 모듈 Questa 개별 시뮬레이션 스크립트
#==============================================================================
# Vivado GUI 없이 Questa GUI만으로 init 모듈 시뮬레이션 실행
#
# Usage:
#   ./run_init.sh         # GUI 모드로 실행 (기본)
#   ./run_init.sh -batch  # 배치 모드로 실행
#
# 사전 요구사항:
...
# 환경 체크
#------------------------------------------------------------------------------
print_header "init 모듈 Questa 시뮬레이션"
...
# 라이브러리 생성
#------------------------------------------------------------------------------
print_info "work_init 라이브러리 생성..."
$VLIB work_init
$VMAP work_init work_init
...
# init 모듈
print_info "컴파일: init.sv"
$VLOG -sv +incdir+$SOURCE_DIR \
    "$SOURCE_DIR/init.sv" \
    -work work_init

# 테스트벤치
print_info "컴파일: init_tb.sv"
$VLOG -sv +incdir+$SOURCE_DIR \
    "$TB_DIR/init_tb.sv" \
    -work work_init
...
if [ "$MODE" == "batch" ]; then
    ...
    # TCL 스크립트 생성
    cat > run_init_gui.tcl << 'EOF'
#==============================================================================
# init 모듈 GUI 시뮬레이션 TCL 스크립트
#==============================================================================
...
#------------------------------------------------------------------------------
print_header "요약"
print_info "모듈: init_tb"
print_info "라이브러리: work_init"
