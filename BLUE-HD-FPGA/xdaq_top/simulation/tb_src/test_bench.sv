...
module test_bench();
	...
	initial begin
		...
		@(negedge test_bench.blue_hd_uut.nRST);
		// @(negedge nRST);
	...
	$display($time, " << prep ack & exp ack done >>");
		...
		wait(test_bench.blue_hd_uut.eim_rst);
		#100;
	...
	#1000000
		$display("[SYS] : Wait 1msec for system stable!");
	...
	#400
	$display($time, " << Reg map sel spi_write >>");
	do_mspi_write(2'b10 , `ADDR_REG_MAP_SEL, 16'h00_01);

	#400
	$display($time, " << LED CTR spi_write >>");
	do_mspi_write(2'b10 , `ADDR_STATE_LED_CTR, 16'h00_4D);

	#400
	$display($time, " << LED CTR spi_read >>");
	do_mspi_read(2'b01, `ADDR_STATE_LED_CTR);

	#400
	$display($time, " << Reg map sel spi_write >>");
	do_mspi_write(2'b10 , `ADDR_REG_MAP_SEL, 16'h00_00);

	#400
	$display($time, " << FSM STATE spi_read >>");
	...
	// while (test_bench.blue_hd_uut.FSM_rst_index !== 1'b1) begin
	// 	@(test_bench.blue_hd_uut.FSM_rst_index); // fsm_rst_index?�� �?? �???���?? 감�?
	// end
...
// #################################
	...
	// while (test_bench.blue_hd_uut.sequence_done_o == 1'b1) begin
	// 	@(!test_bench.blue_hd_uut.sequence_done_o); // fsm_rst_index?�� �?? �???���?? 감�?
	// end

	while (test_bench.blue_hd_uut.panel_stable_o == 1'b1) begin
		@(!test_bench.blue_hd_uut.panel_stable_o); // fsm_rst_index?�� �?? �???���?? 감�?
	end
	...
	// while (test_bench.blue_hd_uut.sequence_done_o == 1'b0) begin
	// 	@(test_bench.blue_hd_uut.sequence_done_o); // fsm_rst_index?�� �?? �???���?? 감�?
	// end
	// $display($time, " << Test Dark 0 Sequence done 0 >>");

	while (test_bench.blue_hd_uut.expose_enable_o == 1'b0) begin
		@(test_bench.blue_hd_uut.expose_enable_o); // fsm_rst_index?�� �?? �???���?? 감�?
	end
	$display($time, " << Test Dark 0 Espose start 0 >>");

	while (test_bench.blue_hd_uut.readout_enable_o == 1'b0) begin
		@(test_bench.blue_hd_uut.readout_enable_o); // fsm_rst_index?�� �?? �???���?? 감�?
	end
	$display($time, " << Test Dark 0 Readout start 0 >>");

	while (test_bench.blue_hd_uut.readout_enable_o == 1'b1) begin
		@(!test_bench.blue_hd_uut.readout_enable_o); // fsm_rst_index?�� �?? �???���?? 감�?
	end
	...
	// while (test_bench.blue_hd_uut.s_exp_read_exist == 1'b1) begin
	// 	@(!test_bench.blue_hd_uut.s_exp_read_exist); // fsm_rst_index?�� �?? �???���?? 감�?
	// end
...
// spi task
    ...
    string roic_data_file_path = "d:/workspace/gittea-work/BLUE-HD-FPGA/xdaq_top/source/hdl/ti-roic/tb_src/roic_sim_data_522.txt"; // Path to ROIC simulation data file
    string roic_256_data_file_path = "d:/workspace/gittea-work/BLUE-HD-FPGA/xdaq_top/source/hdl/ti-roic/tb_src/roic_sim_data_256.txt"; // Path to ROIC simulation data file
    ...
    afe2256_emulator #(
        ...
        .mclk_sim      (mclk_sim),
        .dclk_sim      (dclk_sim),
        .reset         (test_bench.blue_hd_uut.init_rst),
	...
	// assign test_bench.blue_hd_uut.deser_reset = deser_reset;
    ...
    initial begin
        ...
        wait(!test_bench.blue_hd_uut.init_rst); // Wait for initialization reset to complete
	...
	// wait(test_bench.blue_hd_uut.gen_sync_start);
...
//            clk_reset = 1'b1;           // Assert system reset
            ...
            // test_bench.blue_hd_uut.ld_dly_tap = 1'b0;          // Don't load delay taps yet
            // test_bench.blue_hd_uut.in_delay_tap_in = 5'h00;    // Initial delay value = 0
            // test_bench.blue_hd_uut.in_delay_data_ce = 1'b0;    // Disable delay control
            // test_bench.blue_hd_uut.in_delay_data_inc = 1'b0;   // Set delay direction (not used)
            // // Alignment control signals
            // test_bench.blue_hd_uut.extra_shift[0] = 5'h00;        // No additional bit shifting
            // test_bench.blue_hd_uut.align_to_fclk = 1'b0;       // Use auto-detection mode initially
            // test_bench.blue_hd_uut.align_start = 1'b0;         // Don't start alignment yet
    ...
    task configure_delay_settings();
        begin
			// test_bench.blue_hd_uut.in_delay_data_ce = 1'b0;
			// test_bench.blue_hd_uut.in_delay_data_inc = 1'b0;
            // // Configure delay taps for signal timing alignment
            // test_bench.blue_hd_uut.ld_dly_tap = 1'b1;          // Enable loading of tap value
            // test_bench.blue_hd_uut.in_delay_tap_in = 5'h00;    // Optimal tap value (determined empirically)
            // #100;
            // test_bench.blue_hd_uut.ld_dly_tap = 1'b0;          // Disable loading (value is latched)
            // #100;
			do_mspi_write(2'b10 , `ADDR_TI_ROIC_DESER_DLY_DATA_CE, 16'h00_00);
			#100;
			do_mspi_write(2'b10 , `ADDR_TI_ROIC_DESER_DLY_DATA_INC, 16'h00_00);
    ...
    task set_alignment_mode(input logic mode);
        begin
            ...
            if (mode) begin
                // test_bench.blue_hd_uut.align_to_fclk = 1'b1;   // Manual alignment mode with extra_shift value
				do_mspi_write(2'b10 , `ADDR_TI_ROIC_DESER_ALIGN_MODE, 16'h00_01);
				#100;
				$display("[%0t ns] Manual alignment mode enabled", $time);
            end else begin
                // test_bench.blue_hd_uut.align_to_fclk = 1'b0;   // Auto-detection mode using pattern recognition
				do_mspi_write(2'b10 , `ADDR_TI_ROIC_DESER_ALIGN_MODE, 16'h00_00);
				#100;
    ...
    task start_auto_alignment();
        begin
            ...
            // test_bench.blue_hd_uut.align_start = 1'b0;        // Reset alignment trigger first
            // #20;
            // test_bench.blue_hd_uut.align_start = 1'b1;        // Trigger alignment process
            // #200;
			do_mspi_write(2'b10 , `ADDR_TI_ROIC_DESER_ALIGN_START, 16'h00_00);
			#100;
    ...
    task detect_and_save_shift_value(input logic [WORD_SIZE-1:0] new_adc_data = 24'hFFF000);
        begin
            ...
            // test_bench.blue_hd_uut.align_start = 1'b1;        // Reset alignment trigger
			do_mspi_write(2'b10 , `ADDR_TI_ROIC_DESER_ALIGN_START, 16'h00_01);
			#100;

            // Wait for alignment to complete and capture shift_out value
            while(test_bench.blue_hd_uut.align_done[0] == 1'b0) begin
				if (test_bench.blue_hd_uut.align_to_fclk) begin
					// If in manual mode, set the shift value to the detected value
					// test_bench.blue_hd_uut.extra_shift[0] = test_bench.blue_hd_uut.shift_out[0];    // Store the detected shift value
					do_mspi_write(2'b10 , `ADDR_TI_ROIC_DESER_SHIFT_SET_0, {11'd0, test_bench.blue_hd_uut.shift_out[0]});
				end else begin
					// In auto-detection mode, log the detected shift value
					$display("[%0t ns] Detected shift value: %0d", $time, test_bench.blue_hd_uut.shift_out[0]);
					$display("[%0t ns] Detected extra value: %0d", $time, test_bench.blue_hd_uut.extra_shift[0]);
				end
            ...
            // test_bench.blue_hd_uut.align_start = 1'b0;        // Reset alignment trigger
			do_mspi_write(2'b10 , `ADDR_TI_ROIC_DESER_ALIGN_START, 16'h00_00);
			#100; 
    ...
    task switch_to_manual_mode();
        begin
            ...
            // test_bench.blue_hd_uut.align_start = 1'b0;         // First deassert alignment trigger
            // #200;                       // Wait for signal to stabilize
            // test_bench.blue_hd_uut.align_start = 1'b1;         // Restart alignment with new settings
            ...
            wait(test_bench.blue_hd_uut.align_done[0]);
            #200; // Allow time for alignment to stabilize
            // Log confirmed shift value for manual mode
            $display("[%0t ns] Manual mode confirmed with shift value: %0d", $time, test_bench.blue_hd_uut.shift_out[0]);
            test_bench.blue_hd_uut.extra_shift[0] = test_bench.blue_hd_uut.shift_out[0];    // Store the detected shift value            wait(fclk_p); // Wait for frame clock to stabilize
            // test_bench.blue_hd_uut.align_start = ({12{1'b0}});        // Reset alignment trigger
			do_mspi_write(2'b10 , `ADDR_TI_ROIC_DESER_ALIGN_START, 16'h00_00);
			#100; // Allow time for alignment to stabilize
...
//                        $time, r+1, repeat_count);
	...
	initial begin
		...
		forever begin
			...
			// wait(test_bench.blue_hd_uut.FSM_read_index);
			wait(test_bench.blue_hd_uut.align_done[0]);
			sync_counter++;
			...
			// @(negedge test_bench.blue_hd_uut.gen_sync_start);
			// $display($time, " << Sync signal #%0d deasserted - Ready for next sync detection >>", sync_counter);
			
			wait(test_bench.blue_hd_uut.ti_roic_sync);	
