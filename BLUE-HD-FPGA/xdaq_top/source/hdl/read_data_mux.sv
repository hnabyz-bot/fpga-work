...
//    logic [3:0] s_tuser_0_cnt;
    ...
    logic [13:0] read_mem; // Read memory flags
    logic [13:0] read_mem_1d; // Delayed read memory flags
    logic [13:0] read_mem_2d; // 2-cycle delayed read memory flags

    logic [13:0] end_read_mem; // End read memory flags
    logic [13:0] start_read_mem; // Start read memory flags
...
//    assign read_addr_cnt = read_mem_addr_offset_2d;
    ...
    generate
        for (i = 0; i < 14; i++) begin : gen_end_read_mem
    ...
    generate
        for (i = 0; i < 14; i++) begin : gen_start_read_mem
    ...
    // genvar i;
    generate
        for (i = 1; i < 14; i++) begin : gen_start_read
        ...
        if (!eim_rst || tx_eim_rst) begin
            ...
            if (|read_mem_2d) begin
                case (1'b1)
                    ...
                    read_mem_2d[12]: begin
                                        read_data_out_a <= (roic_read_data_a[12]);
                                        read_data_out_b <= (roic_read_data_b[12]);
                                    end
                    read_mem_2d[13]: begin
                                        read_data_out_a <= (roic_read_data_a[13]);
                                        read_data_out_b <= (roic_read_data_b[13]);
                                    end
                    default:        begin
                                        read_data_out_a <= '0;
                                        read_data_out_b <= '0;
